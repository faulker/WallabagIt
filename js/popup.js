...
$(function ()
{
   ...
   wItPop.v2.saveTags = function (item, tags)
   {
      var addTags = $.Deferred();

      wItAuth.getToken().done(function ()
      {
         var feedURL = wItPop.settings.url + '/api/entries/' + item + '/tags.json';
         $.post(feedURL, {
   ...
   wItPop.v2.deleteTag = function (itemId, tagId)
   {
      var delTag = $.Deferred();

      wItAuth.getToken().done(function ()
      {
         var feedURL = wItPop.settings.url + '/api/entries/' + itemId + '/tags/' + tagId + '.json';
   ...
   wItPop.v2.buildTags = function (id)
   {
      var tags     = $(wItPop.cache.tags[id]),
          tagCloud = [];
   ...
   wItPop.feed.load = function (feed)
   {
      ...
      chrome.storage.local.get('wallabagItTags', function (results)
      {
         wItPop.cache.tags = results.wallabagItTags;
         $loading.delay(200).hide(0);
      });
   };
   ...
   $('#btn-tags-save').click(function (e)
   {
      ...
      if (item.length > 0)
      {
         wItPop.v2.saveTags(item, tags);
      }
      else
      {
         console.log('Error: item-id blank');
      }
   ...
   $('body').on('click', '.tag-remove', function (e)
   {
      ...
      wItPop.v2.deleteTag(itemId, tagId);
   ...
   }).on('click', 'a', function ()
   {
      ...
      return false;
   }).on('click', '.glyphicon-tags', function ()
   {
      var $modal   = $('#tags-modal'),
          id = $(this).attr('data-id'),
          tags = wItPop.v2.buildTags(id);

      $modal.find('#item-tags').val('');
      $modal.find('#tags-list').html(tags.join("\n"));
